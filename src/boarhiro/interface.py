...
def _spawn(cmd: list, log_name: str) -> subprocess.Popen:
    ...
    Args:
        ...
        log_name: Name of log file to append output to
...
def _start_ecosystem() -> list:
    ...
    Returns:
        List of status strings describing what was started
    """
    root = _project_root()
    lines = []
...
def print_status(url: str) -> None:
    ...
    if os.path.exists(log_path):
        with open(log_path, encoding="utf-8", errors="replace") as f:
            tail = f.readlines()[-3:]
        lines.append("")
        lines.append("[dim]Recent training logs:[/dim]")
        for log_line in tail:
            lines.append(f"  [dim]{log_line.rstrip()}[/dim]")
...
def ask_boarhiro(prompt: str, url: str) -> str:
    ...
    Args:
        ...
        url: Server URL to send the request to
        
    Returns:
        Response text with rich formatting
    """
    try:
